...
- **AI 味判据**：以下六 Gate 是硬判据，审 aesthetic 维度时必须逐 Gate 对照原文检查并引用违例段落。
  审美审查必须同时覆盖 anti_ai_tone 的 Gate A-F：固定套句、套路句式、告诉代替展示、节奏过整、对话同腔、章末升华。
  ...
  **六 Gate 严重度与处理**：
  | 命中情况 | severity | verdict 影响 |
  |---|---|---|
  | 单个 Gate 轻微命中（1-2 处，不影响阅读） | warning | accept 或 polish（给 targets） |
  | 同一 Gate 在一章内重复 ≥3 次 | error | 至少 polish |
  | 同时命中 ≥3 个不同 Gate | error | 至少 polish，视情况 rewrite |
  | Gate F（章末升华）命中 | error | 至少 polish |
  | Gate A+C+F 同时命中（总结腔+心理标签+升华） | error | rewrite |
  | Gate D+E 同时命中（节奏过整+对话同腔） | error | 至少 polish，给出具体 targets |
  | 整章弥漫 AI 腔（四个以上 Gate 大面积命中） | error | rewrite |
  issue 必须引用原文证据和具体 rule_id/hotspot，不能只写"AI 味重"。
...
以下情况必须判 rewrite：
- 同一章内四个以上 Gate 大面积命中（整章弥漫 AI 腔）
- Gate A+C+F 同时命中，且覆盖章首、章中、章末（总结腔贯穿全文）
- AI 腔问题已导致剧情功能受损（读者无法分辨角色、无法理解因果、被升华句打断情绪）
以下情况优先判 polish + 给出具体 targets：
- Gate F（章末升华）单独命中
- Gate B（套路句式）单类命中 ≥3 次
- Gate D+E 同时命中但仅影响节奏和对话质感，不影响剧情功能
