...
- 世界规则只保留会直接影响剧情的部分
- 结局必须回收核心承诺
- 开头三章内必须给出身份差、冲突、危险、误会、秘密或情绪钩子中的至少一种；不要用长篇式铺设慢热开局
- 反转必须有伏笔和代价，揭示后要改变选择或关系；参考 `reference_pack.references.reversal_toolkit`
- 章首和章尾钩子参考 `reference_pack.references.hook_techniques`，但每个钩子都必须服务单一主冲突
